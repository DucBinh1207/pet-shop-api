...
router.get('/orders/user', authenticateToken, async (req, res) => {
    const id_user = req.user.id; // Lấy id_user từ token
    try {
        ...
        const orders = await ordersCollection.find({ id_user: id_user }).toArray();
...
router.post('/orders/create', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const id_order = orderResult.insertedId; // MongoDB tự động tạo ID
